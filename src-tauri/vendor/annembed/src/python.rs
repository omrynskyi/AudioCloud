...
use crate::prelude::{Embedder, EmbedderParams, get_toembed_from_csv, write_csv_array2};
...
impl HnswArgs {
    fn validate(&self) -> Result<(), &'static str> {
        match self.distance.as_str() {
            ...
            _ => Err(
                "distance must be DistL1 | DistL2 | DistCosine | DistJeffreys | DistJensenShannon",
            ),
        }
    }
}
...
macro_rules! with_dist {
    ($name:expr, $alias:ident, $body:block) => {{
        match $name.as_str() {
            ...
            "DistCosine" => {
                type $alias = DistCosine;
                $body
            }
            "DistJeffreys" => {
                type $alias = DistJeffreys;
                $body
            }
            "DistJensenShannon" => {
                type $alias = DistJensenShannon;
                $body
            }
            _ => {
                type $alias = DistL2;
...
fn build_kgraph(pairs: &[(&Vec<f64>, usize)], hp: &HnswArgs, nb_layer: usize) -> KGraph<f64> {
...
fn embed(
    ...
    if let Some(p) = outfile {
        let mut w =
            csv::Writer::from_path(p).map_err(|e| PyValueError::new_err(format!("{e:?}")))?;
