/**
 * The debounced filter draft `panels/Search.tsx` and `panels/Tags.tsx` write into.
 *
 ...
 *
 * **The draft is three fields now, not five.** It used to mirror most of `QueryFilter` -
 ...
 * rest on the way out); the *interface* only offers the three constraints anyone reaches for
 * while browsing by ear: a name, a tag, or a saved collection. Carrying draft state for
 * controls that no longer exist is how a store quietly becomes fiction.
 */
...
const EMPTY_DRAFT: FilterDraft = { text: '', tags: [], collectionIds: [] };
